...
export default function ScanReport({ data }: { data: ScanReportData }) {
  ...
  const bySeverity = useMemo(() => {
    const groups: Record<Vulnerability['severity'], Vulnerability[]> = {
      CRITICAL: [], HIGH: [], MEDIUM: [], LOW: [], INFO: []
    };
    for (const v of data.vulnerabilities) groups[v.severity].push(v);
    return groups;
  ...
  return (
    <section id="scan-report-root" className="mt-8 sm:mt-10">
      ...
      {/* Vulnerabilities */}
      <div className="mt-6 terminal-border bg-black/80 rounded-xl p-4 sm:p-6">
        <h3 className="text-cyber-400 font-mono text-base mb-3">VULNERABILITIES</h3>
        {data.vulnerabilities.length === 0 && (
          <div className="text-matrix-300 text-sm">No vulnerabilities detected. ✅</div>
        )}
