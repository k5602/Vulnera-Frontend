...
export class OrgSignupOrgData {
    ...
    async sendSignupOrgRequest() {
        ...
        if (result.status !== 201) {
            messageHandler.showError(result.data.message || "Organization creation failed.");
        ...
        setTimeout(() => {
            ...
            console.error('Organization creation error', { error: error instanceof Error ? error.message : String(error) });
            messageHandler.showError(error instanceof Error ? error.message : "Organization creation failed.");
            this.submitBtn.disabled = false;
            this.submitBtn.textContent = "CREATE_ORG";
        }
    }
